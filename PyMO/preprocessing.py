...
#        return X
    ...
    def _to_expmap(self, X):
        ...
        for track in X:
            ...
            euler_df = track.values
            rc = euler_df
            rotation_order = rc.columns[0][rc.columns[0].find('rotation') - 1] + rc.columns[1][
                rc.columns[1].find('rotation') - 1] + rc.columns[2][rc.columns[2].find('rotation') - 1]
            ...
            for joint in joints:
                ...
                euler = [[f[1]['%s_Xrotation'%joint], f[1]['%s_Yrotation'%joint], f[1]['%s_Zrotation'%joint]] for f in r.iterrows()] # Make sure the columsn are organized in xyz order
                 #ZYX but based on my data 'ZXY'
                exps = [Rotation(f, 'euler', 'ZXY',from_deg=True).to_expmap() for f in euler] # Convert the eulers to exp maps
